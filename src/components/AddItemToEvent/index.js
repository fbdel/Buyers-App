...
class AddItemToEvent extends React.Component {
    ...
    saveButtonClick() {
        ...
        if (selectedFile !== null && selectedFile !== undefined && selectedFile !== '') {
            ...
            uploadTask.on('state_changed', function (snapshot) {
                ...
                uploadTask.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                    ...
                    if (!that.state.webcamEnabled) {
                        if (that.isValidString(image) && that.isValidString(vendor) && that.isValidString(price) && that.isValidString(moq)) {
            ...
            filename = Math.random().toString().replace('0.', '');
            storageRef = firebase.storage().ref('/images/' + filename)
            ...
            return (
                <div className="row input-group mb-3">
                    <div className="col-sm-12 text-center">
                        <div>
                            <span className="input-group-text bg-warning" id="basic-addon1">Your Image</span>
                        </div>
            ...
            .then(function () {
                if (document.getElementById('tblEventDetails') !== null && document.getElementById('tblEventDetails') !== undefined) {
                    ...
                    events.map((events, index) => {
                        ...
                        const { eventId, vendor, image, price, moq, comment, description, downPayment, itemName, size } = events;
                        var htmlData = '';
                        htmlData =
                            '<tr key=' + eventId + '>' +
                            '<td>' + vendor + '</td>' +
                            '<td><img height= 50 width= 50 src=' + image + ' /></td>'
                            + '<td>' + moq + '</td>' +
                            '<td>' + comment + '</td>' +
                            '<td>' + description + '</td>' +
                            '<td>' + downPayment + '</td>' +
                      ...
                      <div className="row input-group mb-3">
                        <div className="col-sm-12 text-center">
                          <button
                            id="buttonSave"
                            type="button"
                            className="btn btn-light btn-outline-secondary far fa-save bg-info text-white"
                            style={{
                              width: "150px",
                              height: "50px",
                              marginRight: "5px"
                          ...
                          <button
                            id="buttonReturnHome"
                            type="button"
                            className="btn btn-light btn-outline-secondary fas fa-home bg-info text-white"
                            style={{
                              width: "150px",
                              height: "50px",
                              marginRight: "5px"
